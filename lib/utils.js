...
const getConfig = exports.getConfig = (argv = {}) => {
  ...
  const config = getConfigFromFile(argv.config) || {
    server: {},
  };
  ...
  invariant(config.server.password, 'password is required');
  invariant(config.server.host, 'host is required');
  invariant(config.server.port, 'port is required');
  invariant(config.server.username, 'username is required');

  config.server.username = config.server.username.toString();
  config.server.password = config.server.password.toString();
